...
**Created**: 2026-02-03  
**Status**: Draft  
**Input**: Enhance KNOWLEDGE-BASE with prompt-based semantic search and RAG for the PromptOS skill library
...
- A2: "Semantic similarity" in a prompt-based system means structured multi-signal comparison (name overlap, tag overlap, trigger overlap, description keyword overlap, domain match) weighted and scored by the agent following a rubric.
...
Before any new skill is committed, the agent runs a redundancy check against the existing library. If the new skill overlaps significantly with an existing one, the agent surfaces the overlap and presents three options: expand the existing skill, create a complementary skill, or proceed as-is.

**Why this priority**: Redundancy directly undermines the value of the knowledge base. SKILL-GOVERNANCE.md already defines the policy; this spec provides the *detection* mechanism the agent follows.

**Independent Test**: Submit a skill that is a near-paraphrase of an existing baseline. Verify the agent flags it with similarity ≥ 80 and presents the three options before writing.

**Acceptance Scenarios**:

1. **Given** skill `go` covers goroutines, channels, and interfaces, **When** a draft skill titled "Go Concurrency Patterns" is submitted that covers goroutines and channels, **Then** the agent reports overlap ≥ 80 with `go` and recommends expanding `go` rather than creating a new skill.
2. **Given** a draft skill covers a topic partially covered by skill X (overlap 40-79), **When** the redundancy check runs, **Then** the agent flags it as "complementary" and allows creation with a cross-reference to X.
...
- **FR-002**: The similarity rubric MUST use at least four signals: name/topic overlap, tag overlap, trigger overlap, and description keyword overlap, each with a defined weight.
...
- **FR-005**: The agent MUST run a redundancy check (similarity ≥ 80 = high overlap) against the existing library before any new skill is persisted.
- **FR-006**: When redundancy is detected (≥ 80), the agent MUST present exactly three options: expand existing, create complementary, or proceed as-is — and wait for human approval before writing.
...
- **FR-008**: All retrieval and scoring logic MUST be expressible as prompt instructions — no external service or runtime required for the core feature.
...
- **Similarity Score**: A computed 0-100 value representing how well a skill matches a query or another skill. Composed of four weighted signals (FR-002).
...
- **SC-003**: 100% of skills with similarity ≥ 80 to an existing skill are flagged before persistence. Zero false negatives on a defined test set of 5 near-duplicate drafts.
