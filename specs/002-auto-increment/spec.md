...
3. **Given** a gap was detected and user chooses "proceed without", **When** the agent continues, **Then** the gap is logged in the agent's memory with status "deferred" for future reference
...
**Independent Test**: Can be fully tested by rejecting 5 artifacts with different reasons, verifying (1) the system asks for reason if not given, (2) each rejection is categorized correctly (examples/specificity/clarity/completeness/relevance), (3) all rejections are logged in the agent's memory, and (4) when a category exceeds 30% of rejections, the system proactively mentions it in subsequent generations.

**Acceptance Scenarios**:

1. **Given** an artifact is rejected at Human Gate with reason "Examples don't work", **When** the rejection is processed, **Then** the system categorizes it as "examples", logs it in the agent's memory with timestamp/skill/reason/category, and acknowledges the rejection
...
1. **Given** the topic "Kafka" has been detected as a gap 2+ times in the agent's memory, **When** the user starts a new session or mentions related topics, **Then** the system proactively suggests "I noticed 'kafka' was requested multiple times. Would you like me to create a skill for it?"
...
2. **Given** an evolution report is generated, **When** the user views it, **Then** all metrics are accurate based on aggregated agent memory data and global statistics, and the report is formatted in markdown with tables
...
- **FR-003**: System MUST log every detected gap to the agent's persistent memory with fields: date, user request text, suggested skill name, status (pending/created/deferred/rejected), and timestamps
- **FR-004**: System MUST categorize rejection reasons into one of six categories: examples, specificity, clarity, completeness, relevance, or other
- **FR-005**: System MUST log every rejection to the agent's persistent memory with fields: date, artifact type, artifact name, reason text, category, learned action, and timestamp
...
- **SC-003**: At least 3 skills per month are created directly from gap suggestions (measured as: skills with "created from gap" status aggregated across agent memories)
...
7. **Data Retention**: Historical gap and rejection data in agent memory files is never automatically purged or archived (manual cleanup only)
8. **Single User Context**: Each agent operates within a single user/session context; cross-agent pattern analysis happens only during evolution report generation
